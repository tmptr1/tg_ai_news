...
def get_title():
    ...
    if last_topics:
        ...
        last_topics = f' Новости не должны повторяться, вот массив с предыдущими новостями: {last_topics}).'
...
def send_tg_post(last_send):
    ...
    subjects = ["стройка спортивных объектов", "маркетинг спортивных проектов",
                "сделоки и соглашения в спортивной индустрии", "инвестиций и спонсорства в спорте",
                "кадровые изменения в спорте", "назначения в области менеджмента в спорте",
                "налоговые и юридические изменения в спорте", "исследования в спорте"]
    random.shuffle(subjects)

    request_text = f'''Найди актуальную, интересную новость спортивного бизнеса в России за сегодня (тщательно проверяй дату новости), найди ссылку на первоисточник, напиши пост для телеграмм канала спортивного журнала.{last_titles} Вот массив с примерами новостных тематик: {subjects}. 
    ...
    ai_req_text = f'Проанализируй пост, проверь ссылку на источник и определи: это новость текущего дня или нет? Отвёт помести в переменную res = "да" или res = "нет". Вот текст поста: \n{tg_post}'
    ai_post_check = proxyapi_request(ai_req_text, model='gpt-5-mini')
    logger.log(21, f"{ai_post_check=}")
    res = re.search(r'res?.=?.".+"', ai_post_check).group()
    logger.log(21, f"{res=}")
    res = str.lower(res)
    if not re.search('да', res):
        logger.log(21, "Новость НЕ сегодняшняя!")
        add_title(title)
        return False
        ...
        try:
            ...
            for t in times:
                ...
                if time_now > send_time and (time_now - send_time).seconds / 60 < 5:  # время отправки + 3 мин
                    with open('last_send.txt', 'r') as fr:
                        last_time = datetime.datetime.strptime(fr.read(), '%Y.%m.%d %H:%M:%S')
                        if (time_now - last_time).seconds / 60 > 10: # or last_time.date() != time_now.date():  # если отправлялось больше 10 мин назад
                            if send_tg_post(last_time):
                                with open('last_send.txt', 'w') as fw: # 2026.01.26 09:48:24
                                    fw.write(f"{datetime.datetime.now().strftime('%Y.%m.%d %H:%M:%S')}")

                    # time.sleep(60)
                # else:
                # print('-', t)
        except APIStatusError as api_ex:
            if api_ex.status_code == 402:
                logger.log(21, 'Необходимо пополнить баланс!')
                time.sleep(300)
            else:
                logger.error('api_ex:', exc_info=api_ex)
                time.sleep(30)
...
if __name__ == '__main__':
    ...
    # tg_post = "🏗️ <b>Москва: инвесторы запустили строительство 10 спорткомплексов</b>\n<em>В 2025 году частные инвесторы начали стройку 10 объектов спорта общей площадью 233 тыс. кв. м.</em>\n\n📍 По данным столичного стройкомплекса, новые площадки появятся сразу в нескольких районах — от Ново-Переделкино и Солнцево до Коммунарки, Раменок, Пресненского и других.  \n\n🏊 Среди примеров — ФОК в Ново-Переделкино с тремя бассейнами, объект в ЗелАО (Матушкино) с тренажерным залом и теннисными кортами, а также комплекс в Коммунарке с открытым и закрытым бассейнами и банной зоной.  \n\n📈 Для рынка это хороший сигнал: внебюджетные деньги продолжают заходить в спортинфраструктуру.  \n\n<a href='https://www.interfax-russia.ru/realty/news/stroitelstvo-10-sportkompleksov-startovalo-v-moskve-za-schet-investorov-v-2025-godu'>источник</a>"
    # tg_post = "🏗️ <b>Москва отдаёт спортобъекты в долгосрочную аренду: два комплекса с бассейнами уйдут на торги</b>\n<em>Город рассчитывает, что частные операторы модернизируют площадки и сохранят их спортивный профиль.</em>\n\n📌 По данным Департамента г. Москвы по конкурентной политике, в январе 2026 года пройдут аукционы на право аренды двух спорткомплексов с бассейнами в районах Зябликово и Солнцево сроком на 25 лет.\n\n💼 Инвесторам разрешат провести модернизацию для повышения эффективности, но использовать объекты нужно строго по назначению. А сопутствующие сервисы вроде кафе и вендинга могут занимать не более четверти площади.\n\n⏳ Приём заявок — до 27 января, итоги — 29 января. Подробнее: <a href='https://cre.ru/news/100857'>источник</a>"
    # tg_post = "\n⚽️ <b>Стабильность состава как актив: махачкалинское Динамо продлило контракты с тремя игроками</b>\n<em>Клуб из РПЛ заранее закрывает ключевой кадровый вопрос перед весенней частью сезона.</em>\n\n📌 13 февраля 2026 года махачкалинское Динамо объявило о продлении соглашений с Никитой Глушковым, Идаром Шумаховым и Сосланом Кагермазовым. По данным клуба, их контракты истекали летом 2026-го, а сроки новых договоров не раскрываются.\n\n💼 Для спортивного бизнеса это про снижение рисков: удержание костяка помогает планировать бюджет, трансферную стратегию и работу со спонсорами без лишней турбулентности. 📈\n\n<a href='https://ria.ru/20260213/dinamo-2074199098.html'>источник</a>\n"
    # tg_post = "\n💸 <b>РФС получил €9,4 млн от УЕФА за сезон 2024/25</b>\n<em>Деньги пришли по солидарным и инфраструктурным программам, несмотря на отстранение клубов.</em>\n\n⚽️ По данным Ведомости.Спорт, в сумму вошли: €2,5 млн по программе HatTrick на развитие инфраструктуры и ещё €2,75 млн как поощрение за успехи в ней. \n\n📊 Дополнительно €4,2 млн — выплата за неучастие российских клубов в трёх еврокубках УЕФА. Для рынка это важный сигнал: даже в режиме ограничений сохраняются финансовые механизмы поддержки. 🤝\n\n<a href='https://www.vedomosti.ru/sport/others/articles/2026/02/13/1176084-zoloto-igr-2026-viigral-avstraliets'>источник</a>\n"

    # ai_req_text = f'Проанализируй пост, проверь ссылку на источник и определи: это новость текущего дня или нет? Отвёт помести в переменную res = "да" или res = "нет". Вот текст поста: \n{tg_post}'
    # ai_answer = proxyapi_request(ai_req_text, model='gpt-5-mini')
    # logger.log(21, f"{ai_answer=}")
    # res = re.search(r'res?.=?.".+"', ai_answer).group()
    # logger.log(21, f"{res=}")
    # res = 'res = "нет"'
    # res = str.lower(res)
    # if not re.search('да', res):
